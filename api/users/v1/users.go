...
type RegisterReq struct {
	...
	Name     string `json:"name" v:"required|length:3,12" dc:"用户名"`
...
type LoginReq struct {
	...
	Name     string `json:"name" v:"required|length:3,12"`
	Password string `json:"password" v:"required|length:6,16"`
}

type LoginRes struct {
	Token string `json:"token" dc:"在需要鉴权的接口中header加入Authorization: token" sm:"token格式为Bearer token"`
}
